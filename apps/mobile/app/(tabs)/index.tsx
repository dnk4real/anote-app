...
export default function HomeScreen() {
  ...
  function getFolderLabel() {
    if (currentFolder === 'all') return t('home.allNotes');
    if (currentFolder === 'starred') return t('home.starred');
    if (currentFolder === 'trash') return t('home.recycleBin');
    return folders.find((folder) => folder.id === currentFolder)?.name || t('home.allNotes');
  }
  ...
  async function handlePullToSync() {
    ...
    if (syncNoticeTimerRef.current) clearTimeout(syncNoticeTimerRef.current);
    const providerLabel = syncProvider === 'github' ? t('settings.provider.github') : t('settings.provider.webdav');
    if (result.ok) {
      setSyncNotice({ type: 'success', text: t('home.syncSuccess', { provider: providerLabel }) });
    } else {
      setSyncNotice({ type: 'error', text: result.error || t('home.syncFailed', { provider: providerLabel }) });
    }
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}> 
      <View style={styles.headerRow}>
        ...
        <Pressable
          style={[
            styles.titleBtn,
            language === 'zh' && currentFolder === 'all' && styles.titleBtnZhAllShift,
          ]}
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={[styles.emptyText, { color: colors.textTertiary }]}>{t('home.noNotesYet')}</Text>
        }
      ...
      <DeleteConfirm
        ...
        title={currentFolder === 'trash' ? t('home.deleteNotePermanently') : t('home.moveNoteToRecycleBin')}
        message={
          currentFolder === 'trash'
            ? t('home.noteRemovedPermanentlyMsg')
            : t('home.noteMovedToBinMsg')
        }
        confirmText={currentFolder === 'trash' ? t('common.delete') : t('home.move')}
